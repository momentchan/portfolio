...
/**
 * Generates a texture with uniform lifetime values for all particles
 * @param count - Number of particles (will be rounded to nearest square)
 * @param lifetime - Uniform lifetime value for all particles
 * @param gl - WebGL renderer context
 * @returns DataTexture containing uniform lifetime values
 */
export function generateUniformLifetimeTexture(
    count: number,
    lifetime: number = 5.0,
    gl: THREE.WebGLRenderer
): THREE.DataTexture {
    ...
    // Generate uniform lifetime values for all particles
    for (let i = 0; i < size * size; i++) {
        const index = i * 4;
        data[index] = lifetime;     // lifetime value
        data[index + 1] = 0.0;     // unused
...
/**
 * Generates a texture with gradient lifetime values based on particle position
 * @param count - Number of particles (will be rounded to nearest square)
 * @param minLifetime - Minimum lifetime value
 * @param maxLifetime - Maximum lifetime value
 * @param gl - WebGL renderer context
 * @returns DataTexture containing gradient lifetime values
 */
export function generateGradientLifetimeTexture(
    count: number,
    minLifetime: number = 1.0,
    maxLifetime: number = 5.0,
    gl: THREE.WebGLRenderer
    ...
    // Generate gradient lifetime values based on position
    for (let i = 0; i < size * size; i++) {
        const x = (i % size) / size; // 0 to 1
        const y = Math.floor(i / size) / size; // 0 to 1
        
        // Create gradient from top-left to bottom-right
        const gradient = (x + y) / 2.0;
        const lifetime = minLifetime + gradient * (maxLifetime - minLifetime);
